...
const AdminDashboard = () => {
  ...
  const recentActivity = dummyMembers
    ...
    .map((m) => ({
      name: m.name,
      date: m.attendance[0]?.date,
      status: m.attendance[0]?.status,
    }));
    ...
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-base mt-1">
          Welcome back, Admin. Here's an overview of the congregation.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-10">
        <StatCard
          ...
          sub="Registered in the portal"
          color="bg-primary"
        />
        <StatCard
          icon={faCheckCircle}
          label="Total Attendances"
          value={adminStats.totalAttendances}
          sub="Across all recorded Sundays"
          color="bg-green-500"
        />
        <StatCard
          icon={faCalendar}
          label="This Sunday"
          value={adminStats.thisSunday}
          sub={`of ${adminStats.totalMembers} members present`}
          color="bg-light"
        />
      </div>
